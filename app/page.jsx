...
export default function HomePage() {
  ...
  // -------------------------------------------------
  // 2) جلب المطاعم النشطة + التصنيفات المتاحة من Supabase (بدون أي
  //    شرط على تسجيل الدخول)
  // -------------------------------------------------
  useEffect(() => {
  ...
  const filteredRestaurants = useMemo(() => {
    return restaurants.filter((r) => {
      ...
      const matchesCategory =
        selectedCategories.length === 0 || selectedCategories.includes(r.category_id);
      return matchesQuery && matchesCategory;
    });
  }, [restaurants, query, selectedCategories]);

  const categoryName = (id) => categories.find((c) => c.id === id)?.name || "غير مصنف";
  ...
  {r.image_url ? (
    ...
    <UtensilsCrossed size={32} strokeWidth={1.5} className="text-[#B23E12]" />
                  ...
                  <div className="p-3.5">
                    ...
                    <p className="text-[12.5px] text-[#8A8175] mb-1.5">{categoryName(r.category_id)}</p>
